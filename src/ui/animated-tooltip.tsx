      ...
      {items.map((item, idx) => (
        ...
        >
          {hoveredIndex === item.id && (
            ...
            >
              <div className="absolute bg-primary inset-x-10 z-30 w-auto -bottom-px  h-px " />
              <div className="absolute    left-10 w-[90%] z-30 -bottom-px h-px " />
              <div className="font-bold text-white min-w-[100px] p-1 rounded-2  bg-dark relative z-30 text-base">
                {item.name}
              </div>
              <div className="text-white text-xs  bg-primary ">{item.designation}</div>
            </motion.div>
          )}
          <img
