...
const Orders = () => {
  ...
  useEffect(() => {
    const fetchall = async () => {
      const response = await axiosClient2.get('/api/order/all');
  ...
  useEffect(() => {
    const fetchorders = async () => {
      try {
        setRaph(true);
        const response = await axios.get(`http://127.0.0.1:8000/api/order?page=${currentPage}`);
        setorders(response.data.orders.data);
        setAgain(response.data.orders.data);
        setRaph(false);
  ...
  const handleSearch = () => {
    ...
    const filteredResults = allorders.filter(item =>{
...
useEffect(()=>{
  ...
  const deleteNotification =async ()=>{
    console.log(count)
    await axiosClient2.delete("/api/notification/order");
    setCount(count+20)
    console.log(count)
          ...
          {raph ? (
            ...
            <table className="w-full border border-black border-collapse">
              ...
              <tr>
                ...
                <th className="text-xl text-black font-tech capitalize  py-2 text-center border border-black">Phone</th>
                <th className="text-xl text-black font-tech capitalize  py-2 text-center border border-black">Service</th>
                <th className="text-xl text-black font-tech capitalize  text-center border border-black">order</th>
              ...
              {orders?.map(order => (
                <tr key={order.id} className=" border border-black">
                  <td className="text-xl text-blac kborder border-black   font-tech capitalize  text-center">{order.first_name +" "+order.last_name}</td>
                  <td className="text-sm text-black font-tech capitalize max-w-[270px]  k border border-black  " style={{border:"2px solid black"}}>
                    <div className=' relative w-100 text-xl text-black font-tech capitalize   overflow-x-auto   text-center '>
                      ...
                      {order.email}
                      <button className="copy" onClick={()=>CopyLinkButton(order.email)}>
                  ...
                  <td className="text-sm text-black font-tech capitalize max-w-[270px]  k border border-black  " style={{border:"2px solid black"}}>
                    <div className=' relative w-100 text-xl text-black font-tech capitalize   overflow-x-auto   text-center '>
                      ...
                      {order.phone}
                      <button className="copy" onClick={()=>CopyLinkButton(order.phone)}>
                        <span data-text-end="Copied!" data-text-initial="Copy to clipboard" className="tooltip"></span>
                        <span>
                            <svg xml:space="preserve"    viewBox="0 0 6.35 6.35" y="0" x="0" height="20" width="20" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" xmlns="http://www.w3.org/2000/svg" className="clipboard svg-for-copy">
                            <g>
                  ...
                  <td className=" text-black font-tech capitalize border border-black  text-center max-w-[200px]  overflow-hidden object-cover">
                    <div className='border-none  overflow-x-auto  p-6 h-full w-full'>{order.service}</div>
                  </td>
                  ...
                  <td className="text-xl text-black font-tech h-100 capitalize min-w-[80px]   text-center flex justify-evenly items-center justify-items-center ">
                    <div className={`grid md:grid-cols-1 gap-2 p-1 h-100 flex justify-evenly items-center justify-items-center`}>
                      <motion.button onClick={() => Deleteorder(order.id)} whileHover={{scale: 1.115}}>
