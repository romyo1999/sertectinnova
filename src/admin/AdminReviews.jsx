...
setCount(count+1)
  ...
  useEffect(() => {
    const fetchreviews = async () => {
      try {
        setRaph(true);
        const response = await axios.get(`http://127.0.0.1:8000/api/reviews?page=${currentPage}`);
        setReviews(response.data.reviews.data);
        setRaph(false);

        setAgain(response.data.reviews.data);
  ...
  const Deletereviews = async (id) => {
    confirmAlert({
      ...
      message: 'Are you sure you want to delete this reviews?',
      buttons: [
        {
          label: 'Yes',
          onClick: async () => {
            try {
              ...
              setReviews(reviews.filter(reviews => reviews.id !== id)); // Filter out the deleted reviews
            } catch (error) {
              if (error.response) {
                console.error(error.response.data.error);
              } else {
                console.error('An error occurred while deleting the reviews.');
              }
            }
          }
        },
  ...
  const handleSearch = () => {
    ...
    const filteredResults = allreviews.filter(item =>{
          ...
          {raph ? (
            ...
            <table className="w-full border border-black border-collapse">
              ...
              <tr>
                ...
                <th className="text-xl text-black font-tech capitalize  py-2 text-center border border-black">stars</th>
                <th className="text-xl text-black font-tech capitalize  text-center border border-black">reviews</th>
                <th className="text-xl text-black font-tech capitalize text-center border border-black" >action</th>
              </tr>
              </thead>
              <tbody>
              {reviews?.map(reviews => (
                <tr key={reviews.id} className=" border border-black">
                  <td className="text-xl text-blac kborder border-black   font-tech capitalize  text-center">{reviews.first_name +" "+reviews.last_name}</td>
                  <td className="text-sm text-black font-tech capitalize max-w-[270px]  k border border-black  " style={{border:"2px solid black"}}>
                    <div className=' relative w-100 text-xl text-black font-tech capitalize   overflow-x-auto   text-center '>
                      ...
                      {reviews.email}
                      <button className="copy" onClick={()=>CopyLinkButton(reviews.email)}>
                  ...
                  <td className=" text-black font-tech capitalize border border-black  text-center max-w-[200px]  overflow-hidden object-cover">
                    <div className='border-none  overflow-x-auto  p-6 h-full w-full'>{reviews.stars}</div>
                  </td>
                  ...
                  <td className="text-xl text-black font-tech h-100 capitalize min-w-[80px]   text-center flex justify-evenly items-center justify-items-center ">
                    <div className={`grid md:grid-cols-1 gap-2 p-1 h-100 flex justify-evenly items-center justify-items-center`}>
                      <motion.button onClick={() => Deletereviews(reviews.id)} whileHover={{scale: 1.115}}>
